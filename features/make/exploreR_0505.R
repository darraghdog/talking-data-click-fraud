...
View(trndf[,.(as.integer(100000*mean(is_attributed)), sum(is_attributed), .N), by = device])
...
hist(tstdf[idx]$pred)
hist(subd1$V1)
plot(tstdf[idx]$pred, subd1$V1)
tstdf[idx]$pred = tstdf[idx]$pred*0.4 + subd1$V1*0.6
