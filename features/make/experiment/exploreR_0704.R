...
trndf[, click_hr  := round(as.numeric(click_time)/(3600))%%24]
trndf[, click_qtr  := round(as.numeric(click_time)/(3600))%%6]
tstdf[, click_time := fasttime::fastPOSIXct(tstdf$click_time)]
tstdf[, click_day := round(as.numeric(click_time)/(3600*24))%%31]
tstdf[, click_hr  := round(as.numeric(click_time)/(3600))%%24]

# trndf[device==3032]

aggdf = trndf[,.(.N, mean(is_attributed), sum(is_attributed), length(unique(os))), by = device]
aggdf[N>10000][order(N)]

daydf = trndf[device %in% c(3032, 3543, 3866),.N, by = .(click_day, device)]
...
tstdf[device %in% tst_devs$device,.(.N, length(unique(os))), by = device][order(N)]

tstdf[device == 3, .N , by= click_hr]


trndf[device == 2, .(.N, sum(is_attributed))  , by= .(click_qtr, click_day)][order(click_day, click_qtr)]
trndf[device == 3866, .(.N, sum(is_attributed))  , by= .(click_qtr, click_day)][order(click_day, click_qtr)]
trndf[device == 3543, .(.N, sum(is_attributed))  , by= .(click_qtr, click_day)][order(click_day, click_qtr)]
trndf[device == 3032, .(.N, sum(is_attributed)) , by= .(click_qtr, click_day)][order(click_day, click_qtr)]
