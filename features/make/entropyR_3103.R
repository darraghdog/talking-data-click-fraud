...
# get the entropy over minute, device, app per ip / hour
alldf[, iphr := ip+(click_hr)*1000000 ]
alldf[,.N,by=.(ip, click_hr, iphr)]
length(unique(alldf$iphr))
entropyiphr  <- calc_entropy(alldf, "iphr", 'device', 'iphr_device')
entropyiphr  <- merge(entropyiphr, calc_entropy(alldf, "iphr", 'click_min', 'iphr_click_min'), by = 'iphr', how="all")
hist(entropyiphr$iphr_click_min_entropy)
entropyiphr = merge(alldf[,.N,by=.(ip, click_hr, iphr)], entropyiphr, all = T, by = "iphr")
entropyiphr[,`:=`(N=NULL, iphr=NULL)]
...
writeme(entropyiphr, 'entropyiphr')
