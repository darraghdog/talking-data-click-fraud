...
load_df    = False
debug      = False
train_usecols = ['ip', 'device', 'os', 'app', 'click_time']
test_usecols = ['dataset', 'ip', 'device', 'os', 'app', 'click_time']
...
train_df.drop(['ip', 'app', 'device', 'os', 'category'], axis = 1, inplace = True)
gc.collect()
train_df['nextClick_shift'] = pd.DataFrame(QQ).shift(+1).values
gc.collect()
train_df.drop(['epochtime'], axis = 1, inplace = True)
train_df.fillna(9999, inplace = True)
del QQ
gc.collect()
train_df = train_df.astype(np.int32)
...
print(feattst.shape)
print(feattrn.shape)
feattst.to_feather(path+'../features/feat_next_kanbertst.feather')
feattrn.to_feather(path+'../features/feat_next_kanbertrn.feather')
...
feattrnval.to_feather(path+'../features/feat_next_kanbertrnval.feather')
feattstval.to_feather(path+'../features/feat_next_kanbertstval.feather')
gc.collect()
