...
feattrneip = pd.read_csv(path+'../features/enc_ipdevos_trn%s.gz'%(add_), compression = 'gzip')
feattsteip = pd.read_csv(path+'../features/enc_ipdevos_tst%s.gz'%(add_), compression = 'gzip')
...
feattrn = pd.concat([feattrn, feattrnld2, feattrnspl, feattrneip], axis=1)
feattst = pd.concat([feattst, feattstld2, feattstspl, feattsteip], axis=1)
del feattrnld2, feattrnspl, feattrneip
del feattstld2, feattstspl, feattsteip
gc.collect()
#feattrn.hist()
#feattst.hist()
...
for col in ['app', 'channel', 'channel_app', 'os', 'hour', 'device']:  
    ...
    train_df[col][~train_df[col].isin(common)] = -1
    test_df[col][~test_df[col].isin(common)] =   -1
    val_df  [col][~val_df[col].isin(common)] =   -1
    del common
    gc.collect()
for col in ['app', 'channel', 'channel_app', 'os', 'hour', 'device']:
...
'''
# IP Loo encoding
Auc for all hours in testval : 0.9802923561404003
Auc for select hours in testval : 0.9627574157938313
'''
