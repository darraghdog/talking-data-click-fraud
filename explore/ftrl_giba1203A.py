...
for path in [train, test]:
    for t, row in enumerate(DictReader(open(path))):
        if t%1000000 == 0:
            print("Apps in channel processed rows : %sM; %ss "%( int(t/1e+6), '%0.0f'%(time.time()-start_time)))
        try:
            apps_in_channel[row['app']].add(row['channel'])
        except:
            apps_in_channel[row['app']] = set()
            apps_in_channel[row['app']].add(row['channel'])


#apps_in_channel = dict([(k, v) for (k, v) in apps_in_channel.items() if len(v) > 1])
for k, v in apps_in_channel.items():
    print('App, count of channels, %s %s'%(k, len(v)))
...
def data(path, D):
    ...
    for t, row in enumerate(DictReader(open(path))):
        ...
        #x.append(abs(hash('ip_modulo5_%s'%(int(row['ip'])/5))) % D)
        #x.append(abs(hash('ip_modulo20_%s'%(int(row['ip'])/20))) % D)
        x.append(abs(hash('ip_modulo50_%s_'%(int(row['ip'])/50))) % D)
        #x.append(abs(hash('ip_modulo200_%s_'%(int(row['ip'])/200))) % D)
        # x.append(abs(hash('%s_app_dev_%s'%(row['device'], row['app']))) % D)
        # x.append(abs(hash('%s_os__dev_%s'%(row['device'], row['os']))) % D)
        
        for chl in apps_in_channel[row['app']]:
            x.append(abs(hash('other_channel_%s'%(chl))) %D )
