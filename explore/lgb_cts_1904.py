...
a = collections.Counter(test_df['ip'])
b = collections.Counter(train_df['ip'])
check = set([k for (k,v) in b.items() if v>2000])
len([(k,v) for (k,v) in a.items() if v>2000 if k in check])

len([(k,v) for (k,v) in a.items() if v>200])
len([(k,v) for (k,v) in a.items() if v>10])
len([(k,v) for (k,v) in a.items() if v>1])

np.iinfo(train_df[col])
