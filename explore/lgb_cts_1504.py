...
val_df = val_df[predictors+[target]]
gc.collect()
'''
...
if save_df:
    print('[{}] Load dataset from disk'.format(time.time() - start_time))
    train_df.to_feather(path+'../weights/train_dfval.feather')
    test_df.reset_index(inplace = True, drop = True)
    test_df.to_feather(path+'../weights/test_dfval.feather')
if load_df:
    train_df = pd.read_feather(path+'../weights/train_dfval.feather')
    test_df  = pd.read_feather(path+'../weights/test_dfval.feather')
    feattrncum  = (pd.read_csv(path+'../features/cumsumday_trn%s.gz'%(add_), compression = 'gzip')*10000).astype(np.uint16)
    feattstcum  = (pd.read_csv(path+'../features/cumsumday_tst%s.gz'%(add_), compression = 'gzip')*10000).astype(np.uint16)
    gc.collect()
    train_df = pd.concat([train_df, feattrncum], axis=1)
    test_df  = pd.concat([test_df,  feattstcum], axis=1)
    val_df = test_df#.sample(frac=.25, replace=False, random_state=0)
    gc.collect()
