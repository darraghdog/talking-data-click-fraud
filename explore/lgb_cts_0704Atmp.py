...
feattrnctn1  = pd.read_csv(path+'../features/lead_count_next_ipdevosapp_trn%s.gz'%(add_), compression = 'gzip').astype(np.int16)
feattstctn1  = pd.read_csv(path+'../features/lead_count_next_ipdevosapp_tst%s.gz'%(add_), compression = 'gzip').astype(np.int16)
feattrnctn2  = pd.read_csv(path+'../features/lead_count_next_ipdevos_trn%s.gz'%(add_), compression = 'gzip').astype(np.int16)
feattstctn2  = pd.read_csv(path+'../features/lead_count_next_ipdevos_tst%s.gz'%(add_), compression = 'gzip').astype(np.int16)
feattrnprev  = pd.read_csv(path+'../features/prevdayipchlqtytrn%s.gz'%(add_), compression = 'gzip')#.astype(np.int32)
feattstprev  = pd.read_csv(path+'../features/prevdayipchlqtytst%s.gz'%(add_), compression = 'gzip')#.astype(np.int32)
...
train_df = pd.concat([train_df, feattrn, feattrnnext, feattrnprev, feattrnctn1, feattrnctn2], axis=1)
test_df  = pd.concat([test_df , feattst, feattstnext, feattstprev, feattstctn1, feattstctn2], axis=1)
...
print('[{}] Drop features begin'.format(time.time() - start_time))
drop_cols = [c for c in train_df.columns if c not in ['is_attributed', 'click_id'] + predictors]
...
#[50]    train's auc: 0.9804     valid's auc: 0.97485
#[100]   train's auc: 0.984153   valid's auc: 0.979414
#[200]   train's auc: 0.986082   valid's auc: 0.981666
#[300]   train's auc: 0.986917   valid's auc: 0.982365
#[400]   train's auc: 0.987332   valid's auc: 0.982725
#[500]   train's auc: 0.987669   valid's auc: 0.982903
#[600]   train's auc: 0.987926   valid's auc: 0.982981
#[700]   train's auc: 0.988143   valid's auc: 0.983
#[800]   train's auc: 0.988336   valid's auc: 0.983041
#[900]   train's auc: 0.988514   valid's auc: 0.983032
...
#Model Report
#('n_estimators : ', 909)
#('auc:', 0.9830529979764215)
#                                feat   imp
#0                            channel  1460
#1                                 os   915
#2                                app   801
#3           click_sec_lead_split_sec   265
#4                      ip_os_entropy   147
#5                  ip_device_entropy   138
#6                               hour   133
#7                 ip_channel_entropy   128
#8               ip_click_min_entropy   112
#9                            qty_chl    94
#10               ip_click_hr_entropy    94
#11                               qty    93
#12                click_sec_lead_app    84
#13                        day_minute    79
#14                    ip_app_entropy    76
#15             unique_app_ipdevosmin    70
#16                      ip_app_count    64
#17                                ip    63
#18  click_sec_lead_split_sec_ip_only    60
#19            count_in_next_ten_mins    56
#20                   ip_app_os_count    55
#21                            device    55
#22                       prevday_qty    52
#23                 click_sec_lag_app    48
#24                  click_sec_lag_os    43
#25                      prevhour_qty    42
#26                  count_in_next_hr    39
#27             click_sec_lead_shift2    38
#28                 click_sec_lead_os    33
#29     count_in_next_ipdevos_ten_min    30
#30                click_sec_lead_chl    28
#31                 click_sec_lag_chl    27
#32     count_in_next_ipdevos_ten_sec    22
#33         click_sec_lead_sameappchl    10
#Auc for all hours in testval : 0.9830529979764215
#Auc for select hours in testval : 0.9665836910499783
#writing...
