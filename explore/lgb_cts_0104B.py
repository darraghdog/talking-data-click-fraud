...
feattrnprev  = pd.read_csv(path+'../features/prevqdayipchlqtytrn%s.gz'%(add_), compression = 'gzip').astype(np.int32)
feattstprev  = pd.read_csv(path+'../features/prevqdayipchlqtytst%s.gz'%(add_), compression = 'gzip').astype(np.int32)
...
lead_cols += [col for col in train_df.columns if 'next' in col]
lead_cols += [col for col in train_df.columns if 'entropy' in col]
lead_cols += [col for col in train_df.columns if 'prev' in col]
...
if not validation:
    ...
    print("writing...")
    sub.to_csv(path + '../sub/sub_lgb0104.csv.gz',index=False, compression = 'gzip')
    print("done...")
    print(sub.info())
else:
    max_ip = 126413
