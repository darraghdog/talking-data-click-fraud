...
feattrnnsp = pd.read_csv(path+'../features/not_seen_prev_trn%s.gz'%(add_), compression = 'gzip')
feattstnsp = pd.read_csv(path+'../features/not_seen_prev_tst%s.gz'%(add_), compression = 'gzip')
...
feattrn = pd.concat([feattrn, feattrnspl, feattrnsp1, feattrncum, feattrnchl, feattrnos, feattrnld2, feattrnnsp], axis=1)
feattst = pd.concat([feattst, feattstspl, feattstsp1, feattstcum, feattstchl, feattstos, feattstld2, feattstnsp], axis=1)
...
lead_cols += [col for col in train_df.columns if '_count' in col]
lead_cols += [col for col in train_df.columns if '_var' in col]
lead_cols += [col for col in train_df.columns if '_mean' in col]
...
lead_cols += [col for col in train_df.columns if 'not_seen_' in col]
...
if not validation:
    ...
    print("writing...")
    sub.to_csv(path + '../sub/sub_lgb0405.csv.gz',index=False, compression = 'gzip')
    print("done...")
    print(sub.info())
else:
    max_ip = 126413
    ...
    predsdf.to_csv(path + '../sub/sub_lgb0405val.csv.gz',index=False, compression = 'gzip')
