...
print('[{}] Load up the kabir features'.format(time.time() - start_time))
feattrnkan1 = pd.read_feather(path+'../features/feat_next_kabirtrn%s.pkl'%(add_))
feattstkan1 = pd.read_feather(path+'../features/feat_next_kabirtst%s.pkl'%(add_))
...
feattrn = pd.concat([feattrn, feattrnkan1, feattrnspl, feattrnsp1, feattrncum, feattrnchl, feattrnos, feattrnld2], axis=1)
feattst = pd.concat([feattst, feattstkan1, feattstspl, feattstsp1, feattstcum, feattstchl, feattstos, feattstld2], axis=1)
...
lead_cols += [col for col in train_df.columns if 'count' in col]
lead_cols += [col for col in train_df.columns if '_cumcount' in col]
lead_cols += [col for col in train_df.columns if '_countuniq' in col]
...
if not validation:
    ...
    print("writing...")
    sub.to_csv(path + '../sub/sub_lgb0105.csv.gz',index=False, compression = 'gzip')
    print("done...")
    print(sub.info())
else:
    max_ip = 126413
    ...
    predsdf.to_csv(path + '../sub/sub_lgb0105val.csv.gz',index=False, compression = 'gzip')
