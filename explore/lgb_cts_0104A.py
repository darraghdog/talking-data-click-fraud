...
for col in ['app', 'channel', 'os', 'hour', 'device']:  #'channel_app', 
    gc.collect()
    print('Get common to train and test : %s'%(col))
    # val_cts = train_df[col].value_counts()
    # big_vals = val_cts[val_cts>100].index.tolist()
    common = pd.Series(list(set(train_df[col]) & set(test_df[col])))
    # common = common[common.isin(big_vals)]
