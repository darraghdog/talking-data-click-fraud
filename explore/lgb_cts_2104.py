...
train_df['hour'] = pd.to_datetime(train_df.click_time).dt.hour.astype(np.int32)
...
for col in ['app', 'channel', 'os', 'hour', 'device']:  
    ...
    train_df[col][~train_df[col].isin(keep)] = -1
    test_df[col][~test_df[col].isin(keep)] = -1
    val_df  [col][~val_df[col].isin(keep)] = -1
...
if not validation:
    ...
    print("writing...")
    sub.to_csv(path + '../sub/sub_lgb2104.csv.gz',index=False, compression = 'gzip')
    print("done...")
    print(sub.info())
else:
    max_ip = 126413
    ...
    predsdf.to_csv(path + '../sub/sub_lgb2104val.csv.gz',index=False, compression = 'gzip')
