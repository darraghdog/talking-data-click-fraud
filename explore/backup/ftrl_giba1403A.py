...
trainlead1 = data_path + 'lead_lag_trn_ip_device_os_channelvalsmall.gz'
testlead1  = data_path + 'lead_lag_tst_ip_device_os_channelvalsmall.gz'
trainlead2 = data_path + 'lead_lag_trn_ip_device_osvalsmall.gz'
testlead2  = data_path + 'lead_lag_tst_ip_device_osvalsmall.gz'
...
def getgz_leads(path):
    trnlead = []
    for t, row in enumerate( gzip.open(path, 'r') ):
        if t==0:
            continue # skip header
        if t%10000000 == 0:
            print("Lead trn Processed Rows: %sM; %ss "%( int(t/1e+6), '%0.0f'%(time.time()-start_time)))
        lead, lag = row.decode("utf-8") .replace('\n', '').split(',')
        trnlead.append((bucket(lead), bucket(-int(lag))))
    return  trnlead



trlead1 = getgz_leads(trainlead1)
trlead2 = getgz_leads(trainlead2)
tslead1 = getgz_leads(testlead1)
tslead2 = getgz_leads(testlead2)
print(trlead1[:5])

import collections
print(collections.Counter(trlead1[:100]))
...
def data(path, D, leadls1, leadls2):
    ...
    for t, row in enumerate(DictReader(open(path)) ):
        ...
        hour_, min_, sec_ = time.split(':')
        x.append(abs(hash('hour_%s'%(hour_))) % D)
        # x.append(abs(hash('hour_min_%s_%s'%(hour_, min_))) % D)
        del row['click_time']
        ...
        x.append(abs(hash('ip_modulo50_%s_'%(int(row['ip'])/50))) % D)
        lead, lag = leadls1[t]
        x.append(abs(hash('1lead_%s'%(int(lead)))) % D)
        x.append(abs(hash('1lag_%s'%(int(lag)))) % D)
        x.append(abs(hash('1lag_lead_%s_%s'%(int(lag), int(lead)))) % D)
        lead, lag = leadls2[t]
        x.append(abs(hash('2lead_%s'%(int(lead)))) % D)
        x.append(abs(hash('2lag_%s'%(int(lag)))) % D)
        x.append(abs(hash('1lag_lead_%s_%s'%(int(lag), int(lead)))) % D)
...
for e in range(epoch):
    ...
    for t, x, y, date, _ in data(train, D, trlead1, trlead2):  # data is a generator
...
with open(submission, 'w') as outfile:
    ...
    for t, x, y, date, click_id in data(test, D, tslead1, tslead2):
        p = learner.predict(x)
        outfile.write('%s,%s\n' % (click_id, str(p)))
        if t%1000000 == 0:
