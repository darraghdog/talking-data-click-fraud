...
train_df['os_channel'] = train_df['channel'] + 500*train_df['os']
train_df['os_app']     = train_df['app'] + 500*train_df['os']
...
predictors = ['os_channel', 'os_app', 'channel_app', 'ip', 'app','device','os', 'channel', 'hour', 'qty', 'ip_app_count', 'ip_app_os_count'] + lead_cols
categorical = ['os_channel', 'os_app', 'channel_app', 'app','device','os', 'channel', 'hour']
print(50*'*')
print(predictors)
print(50*'*')
print(categorical)
...
if not validation:
    ...
    print("writing...")
    sub.to_csv(path + '../sub/sub_lgb2503.csv.gz',index=False, compression = 'gzip')
    print("done...")
    print(sub.info())
else:
    max_ip = 126413
...
'''
Auc for all hours in testval : 0.9750223242849518
Auc for select hours in testval : 0.9503251499606864
''' 
'''
# Original
