...
feattrnos  = pd.read_csv(path+'../features/lead_lag_trn_ip_device_os%s.gz'%(add_), compression = 'gzip', dtype=lagdtypes)
feattstos  = pd.read_csv(path+'../features/lead_lag_tst_ip_device_os%s.gz'%(add_), compression = 'gzip', dtype=lagdtypes)
feattrnct  = pd.read_csv(path+'../features/counttrn%s.gz'%(add_), dtype=ctdtypes, compression = 'gzip')
feattstct  = pd.read_csv(path+'../features/counttst%s.gz'%(add_), dtype=ctdtypes, compression = 'gzip')
...
for col in ['app', 'channel', 'channel_app', 'ip', 'os', 'hour', 'device']:  
    ...
    del common
    gc.collect()
train_df['ip'][~train_df['ip'].isin(common_ips)] = np.nan
...
print('[{}] Writing ... '.format(time.time() - start_time))
sub.to_csv(path + '../sub/sub_lgb2403%s.csv'%(add_),index=False, compression = 'gzip')
