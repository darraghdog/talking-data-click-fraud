...
def bucket(val):
    if val < 5:
        return val+2
    for cut in [8, 10, 15, 30, 60, 300, 1600, 7200, 96000]:
        if val < cut:
            return cut
    return 99999999
...
def get_leads(path):
    ...
    for t, row in enumerate(DictReader(open(path))):
        ...
        trnlead.append(int(row['click_sec_lead']))
    return list(map(bucket, trnlead))

trlead = get_leads(trainlead)
tslead = get_leads(testlead)
print(trlead[:5])

import collections
print(collections.Counter(trlead))
...
def data(path, D, leadls):
    ...
    for t, row in enumerate(DictReader(open(path)) ):
        ...
        lead = leadls[t]
        x.append(abs(hash('lead_%s'%(int(lead)))) % D)
...
for e in range(epoch):
    ...
    for t, x, y, date, _ in data(train, D, trlead):  # data is a generator
...
with open(submission, 'w') as outfile:
    ...
    for t, x, y, date, click_id in data(test, D, tslead):
        p = learner.predict(x)
        outfile.write('%s,%s\n' % (click_id, str(p)))
        if t%1000000 == 0:
