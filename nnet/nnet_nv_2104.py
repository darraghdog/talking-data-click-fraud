...
from keras.layers import BatchNormalization, SpatialDropout1D, Conv1D, Add, Average, multiply
...
deepfmcols = ['app', 'channel', 'device', 'os', 'hour']
...
cont_dense = Dense(50, activation="relu", name = 'dense_continuous')(cont_concat)
cont_dense = Dropout(0.3, name = 'dropout_continuous')(cont_dense)
...
for inter_ in list(itertools.combinations(deepfmcols, 2)):
    emb_model['_'.join(inter_)] = multiply([emb_model[inter_[0]], emb_model[inter_[1]] ])

fe = concatenate([(emb_) for emb_ in emb_model.values()])
s_dout = SpatialDropout1D(0.3)(fe)
conv_layers = dict(( ('conv'+str(i), Conv1D(int(200/i), kernel_size=2**i, strides=i-1, padding='same', name = 'conv'+str(i))(s_dout)) for i in range(2,6) ))
...
ftext_emb = dict(([( col , Embedding(embedding_max[col]+1, embed_size \
        , weights = [embedding_matrix[col]], trainable = False, name = 'ftext_emb_'+col) (emb_inputs[col]) ) for col in mapper.keys() if col != 'hour']))
#ftext_emb_avg = Average(name = 'ftext_layers_average')([ emb for emb in ftext_emb.values() ])
ftext_concat = concatenate([(emb_) for emb_ in ftext_emb.values()])
ftext_dout = SpatialDropout1D(0.4)(ftext_concat)
ftext_conv_layers = dict(( ('ftext_conv'+str(i), Conv1D(int(200/i), kernel_size=2**i, strides=i-1, padding='same', name = 'ftext_conv'+str(i))(ftext_dout)) for i in range(3,6) ))
ftext_flatten_layers = dict((  ('ftext_flatten_conv'+str(i), Flatten(name = 'ftext_flatten_conv'+str(i))(ftext_conv_layers['ftext_conv'+str(i)]))  for i in range(3,6) ))
'''
ftext_dense = Dense(100, activation="relu", name = 'dense_fttext')(ftext_conv_layers)
ftext_dense = Dropout(0.5, name = 'dropout_fttext')(ftext_dense)
ftext_dense_flat = Flatten()(ftext_dense)
'''

concat = concatenate([(f_inp) for f_inp in flatten_layers.values()] + [cont_dense] + [(ft_inp) for ft_inp in ftext_flatten_layers.values()], name = 'concat_all')
...
if not validation:    
    ...
    del test_df; gc.collect()
    if stack_val:
        sub.to_csv(path + '../sub/sub_nnet2104_stack_val.csv.gz',index=False, compression = 'gzip')       
    else:
