...
conv1 = Conv1D(400, kernel_size=4 , strides=1, padding='same', name = 'conv1')(s_dout)
conv2 = Conv1D(200, kernel_size=8 , strides=1, padding='same', name = 'conv2')(conv1)
conv3 = Conv1D(100, kernel_size=16, strides=1, padding='same', name = 'conv3')(conv2)
conv4 = Conv1D(50,  kernel_size=32, strides=1, padding='same', name = 'conv4')(conv3)
...
concat = concatenate([(flatten_layer)] + [(c_inp) for c_inp in cont_inputs.values()])
x = Dropout(0.4)(Dense(dense_n, activation='relu')(concat))
x = Dropout(0.4)(Dense(int(dense_n/4) ,activation='relu')(x))
