...
embsz = dict([(c, 100) if '_bins' in c else (c, 200) for c in embids])
...
emb_sum = L.Lambda(lambda x: K.sum(x, axis=0), name = 'emb_sum_cat_cols')([t for t in emb_model.values() if '_bins' not in t.name])
emb_model['emb_sum_binned_cols'] = embbin_sum
emb_model['emb_sum_cat_cols'] = emb_sum
# Concat the sum of the contiuous with the categorical
fe = concatenate([(emb_) for emb_ in emb_model.values() if 'emb_sum_' in emb_.name])
# Rest of the model
...
#plot_model(model, to_file=path+'../nnet/plot/model_nnet1404B.png', show_shapes = True)
...
if validation:
    for i in range(epochs):
        ...
        if epochs - i <= blend_epochs:
            ...
            model.save_weights(path + '../weights/nnet_wts_1404B_data_epoch%s_%s.h5'%(i, 'val'))
    ...
    for i in range(epochs):
        ...
        if epochs - i <= blend_epochs:
            print('[{}] Predicting'.format(time.time() - start_time))
            predsls.append(model.predict(test_df, batch_size=batch_size, verbose=2))
            model.save_weights(path + '../weights/nnet_wts_1404B_data_%s_%s.h5'%(i, 'full'))
    preds = sum(predsls)/len(predsls)
