...
def transform_lead(df, bins = 200, nafillfrom = -1, nafillto = 7200):
    all_cols = df.columns
    for col in all_cols :
...
print('[{}] Load cumulative sum'.format(time.time() - start_time))
...
featcum     = pd.concat([feattrncum, feattstcum])
del feattrncum, feattstcum
featcum.fillna(-1, inplace = True)
featcum = transform_lead(featcum)
featcum['cumsum10_bins'].value_counts()

print('[{}] Load lead2'.format(time.time() - start_time))
...
train_df = pd.concat([train_df, featapp, featspl, featctn, featcum, featld2], axis = 1)
...
if not validation:    
    ...
    del test_df; gc.collect()
    sub.to_csv(path + '../sub/sub_nnet0505.csv.gz',index=False, compression = 'gzip')
    print(sub.info())
    ...
    print('Auc for all hours in testval : %s'%(metrics.auc(fpr, tpr)))
    sub['is_attributed'] = preds
    sub.to_csv(path + '../sub/sub_nnet0505val.csv.gz',index=False, compression = 'gzip')
    print('[{}] All done ...'.format(time.time() - start_time))
